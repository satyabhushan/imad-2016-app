...
app.get('/887/:id',function(req,res){
    //if(req.session && req.session.auth && req.session.auth.user){
        pool.query('SELECT a.artid,a.arttit,a.artdis,a.arttime,a.userid,c.tagid,c.tagname FROM articles a left JOIN tagscon b ON a.artid = b.tagartid left JOIN tags c on b.tagid=c.tagid',function(err,result){
            if(err){
                //res.send('c working');
            }else{
                res.send(JSON.stringify(result.rows));
